        ...
        core_1.json('package.json', (data) => {
            return {
                ...
                scripts: utils_1.sortObjectKeys({
                    ...
                    'build:client': 'env-cmd -f ./.env.production -x --use-shell "parcel build src/client/index.html --public-url $PUBLIC_URL --no-source-maps --out-dir bld/client"',
                    'build:server': 'tsc --P ./src/server/tsconfig.json',
                    dev: 'tsnd --respawn --P ./src/server/tsconfig.json --T ./src/server/main.ts ',
                    start: 'env-cmd -f ./.env.development -x --use-shell "parcel src/client/index.html --open -p $CLIENT_PORT --out-dir bld/client"',
                }, 'asc'),
