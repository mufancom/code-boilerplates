...
const composable = async (options) => {
    ...
    let packagesWithEntrancesEnabledProject = lodash_1.default.uniqBy(projects.map(project => project.package), packageOptions => packageOptions.packageJSONPath);
    let dependencies = await utils_1.fetchPackageVersions(DEPENDENCY_DICT);
    return packagesWithEntrancesEnabledProject.map(packageOptions => core_1.json(packageOptions.packageJSONPath, (data) => {
        return {
            ...data,
            dependencies: {
                ...data.dependencies,
